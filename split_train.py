...
def norm_loss_with_normalization(y_pred, y, alpha=[1, 1], p=1, q=2, detach=False, exponent=True):
...
def finetune_epoch(ft_loader, model, model_ema, optimizer, scheduler, device, epoch=-1, 
    ...
    for i, data in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        p_loss, r_loss = norm_loss_with_normalization(y_pred, y), rank_loss(y_pred, y)
        ...
        if need_separate_sup:
            p_loss_a = norm_loss_with_normalization(scores[0].mean((-3, -2, -1)), y)
            p_loss_b = norm_loss_with_normalization(scores[1].mean((-3, -2, -1)), y)
        ...
        if need_upsampled:
            ...
            if need_separate_sup:
                p_loss_up_a = norm_loss_with_normalization(scores_up[0].mean((-3, -2, -1)), y)
                p_loss_up_b = norm_loss_with_normalization(scores_up[1].mean((-3, -2, -1)), y)
            ...
            p_loss_up, r_loss_up = norm_loss_with_normalization(y_pred_up, y), rank_loss(y_pred_up, y)
            ...
            if need_fused:
                ...
                #print(y_pred_f)
                p_loss_f, r_loss_f = norm_loss_with_normalization(y_pred_f, y), rank_loss(y_pred_f, y)
