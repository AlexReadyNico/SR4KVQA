...
def finetune_epoch(ft_loader, model, model_ema, optimizer, scheduler, device, epoch=-1, 
    ...
    for i, data in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        p_loss, r_loss = plcc_loss(y_pred, y), norm_loss_with_normalization(y_pred, y)
        ...
        if need_upsampled:
            ...
            #----------Ready
            p_loss_up, r_loss_up = plcc_loss(y_pred_up, y), norm_loss_with_normalization(y_pred_up, y)
            #----------Ready
            # loss += p_loss_up + 0.1 * r_loss_up
            loss += (p_loss + r_loss_up)
            wandb.log(
                {
            ...
            if need_fused:
                ...
                #print(y_pred_f)
                p_loss_f, r_loss_f = plcc_loss(y_pred_f, y), norm_loss_with_normalization(y_pred_f, y)
                #----------Ready
                #loss += 0.25 * (p_loss_f + 0.1 * r_loss_f)
                loss += (p_loss + r_loss_f)               
                wandb.log(
                    {
